...
if SYSTEM == "Darwin":
    SOURCE_DIR = os.path.join(MAC, "samples/granulation")
    OUT = os.path.join(MAC, "out")
    DB = os.path.join(MAC, "grains.sqlite3")
    
elif SYSTEM == "Linux":
    SOURCE_DIR = os.path.join(ARGON, "samples/granulation")
    OUT = os.path.join(ARGON, "out")
    DB = os.path.join(ARGON, "grains.sqlite3")

else:
    SOURCE_DIR = os.path.join(PC, "samples\\granulation")
    OUT = os.path.join(PC, "out")
    DB = os.path.join(PC, "grains.sqlite3")


def render(grain_entry_categories, num_unique, num_channels, source_dir, out_dir, name):
    """
    ...
    :param source_dir: The location of the audio files
...
if __name__ == "__main__":
    ...
    NUM_AUDIO_CANDIDATES = 20
    NUM_CHANNELS = 8
    NUM_UNIQUE_GRAINS = 10
    processes = [mp.Process(target=render, args=(grain_entry_categories, NUM_UNIQUE_GRAINS, NUM_CHANNELS, SOURCE_DIR, OUT, f"out_{i+1}.wav")) for i in range(NUM_AUDIO_CANDIDATES)]
    for p in processes:
        p.start()
    for p in processes:
        p.join()
